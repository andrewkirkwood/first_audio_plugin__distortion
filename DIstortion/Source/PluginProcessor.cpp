...
//            *channelData = (((((2.f / float_Pi) * atan(*channelData)) * blend) + (cleanSignal * (1.f / blend))) / 2) * volume;
            ...
            *channelData = ((((2.f / float_Pi) * atan(*channelData)) * blend) + (cleanSignal * (1.f - blend))) * volume;
            
            channelData++;
            
//            @1:05:37 the correct way for dry/wet control is: (distorted * blend) + (clean * (1-blend)) instead of ((distorted * blend) + (clean / blend)) /2
//
//blend + (1-blend) have to be always = 1, otherwise you apply gain here
//
//just try with 50% dry/wet:
//blend = 0.5 -> (distorted * 0.5) + (clean * (1-0.5)) = (distorted * 0.5) + (clean * 0.5)
//
//your formula with 50%:
//blend = 0.5  -> ((distorted * 0.5) + (clean / 0.5)) /2 = (distorted * 0.25) + (clean * 1)
        }
    }
}
